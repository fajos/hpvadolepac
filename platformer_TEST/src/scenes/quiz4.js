...
class quiz4 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// question
		const question = this.add.bitmapText(24, 30, "press2start", "What is the age of\npersons to be vaccinated?");
		question.text = "What is the age of persons\nto be vaccinated?";
		question.fontSize = 9;
		...
		// text1
		const text1 = this.add.bitmapText(30, 64, "press2start", "All ages");
		text1.text = "A.All ages";
		text1.fontSize = 8;

		// text2
		const text2 = this.add.bitmapText(150, 64, "press2start", "9-26 years old");
		text2.text = "B.9-26 years\nold";
		text2.fontSize = 8;

		// text
		const text = this.add.bitmapText(30, 114, "press2start", "Sexually active\n16-45 years");
		text.text = "C.Sexually\nactive\n16-45 years";
		text.fontSize = 8;

		// text_1
		const text_1 = this.add.bitmapText(150, 114, "press2start", ">45 years old");
		text_1.text = "D.>45 years\nold";
		text_1.fontSize = 8;
		...
		const lvl = this.scene.get("Level");
  		lvl.currentAttempt++;
   		window.parent.postMessage(
		     { event: 'retry', level: 'level4', attempt: this.scene.get('Level').currentAttempt },
     			'*'
   		);
	...
	continue = () => {
  	...
  	// tell the parent we’ve started level5
  		window.parent.postMessage(
    	{ event: 'level_started', level: 'level5', attempt: 0 },
    	'*'
  	);
